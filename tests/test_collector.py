...
ROOT = pathlib.Path(__file__).resolve().parents[1] / 'collector'


def load(name):
    spec = importlib.util.spec_from_file_location(f'jade_{name}', ROOT / f'{name}.py')
...
class Publish(unittest.TestCase):
    def setUp(self):
        ...
        patcher = mock.patch.object(update, 'RECORDS', self.records)
        patcher.start()
    ...
    def test_records_are_published_without_leftover_temp_files(self):
        ...
        with mock.patch.object(update, 'collect', side_effect=fake):
            self.assertEqual(update.main(['--force']), 0)
