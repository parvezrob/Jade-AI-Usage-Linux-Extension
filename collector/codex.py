...
def cache_root():
  ...
  root = Path(os.environ.get("XDG_CACHE_HOME") or (Path.home() / ".cache")) / "osaka-ai-usage" / "scan"
...
def fetch_codex_rpc():
  ...
  try:
    rpc_request(proc, 1, "initialize", {"clientInfo": {"name": "jade-ai-usage", "version": "1"}}, timeout=8)
