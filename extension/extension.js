...
const ALARM = 0.9;
// Sit just right of Astra Monitor's readouts when it is running.
const PANEL_NEIGHBOUR = 'monitor@astraext.github.io';
const LIMITS_PROBE_GAP_S = 30;
const VERTICAL = Clutter.Orientation.VERTICAL;

// Osaka Jade, as cairo RGB. CSS carries the same values for St widgets.
const RGB = {
    fg: [0.757, 0.769, 0.592],
    jade: [0.506, 0.722, 0.659],
    track: [0.196, 0.278, 0.231],
    urgent: [0.859, 0.624, 0.612],
};
const HEX = {jade: '#81B8A8', urgent: '#db9f9c', muted: '#ABB8A5'};
...
export default class JadeAIUsage extends Extension {
    enable() {
        this._alive = true;
        this._records = {};
        ...
        this._collector = GLib.build_filenamev([GLib.get_home_dir(), '.local', 'lib', 'osaka-ai-usage', 'update.py']);
        this._recordsDir = Gio.File.new_for_path(GLib.build_filenamev([GLib.get_user_cache_dir(), 'osaka-ai-usage', 'records']));
        ...
        this._settingsChanged = this._settings.connect('changed', () => this._renderPanel());
    ...
    // Right after the neighbour if it is already in the panel; otherwise
    // first, so a neighbour added later still lands on our left.
    _panelIndex() {
        const neighbour = Main.panel.statusArea[PANEL_NEIGHBOUR]?.container;
        return neighbour ? Main.panel._rightBox.get_children().indexOf(neighbour) + 1 : 0;
    }

    _gicon(name) {
        return new Gio.FileIcon({file: this.dir.get_child('icons').get_child(name)});
    }
    ...
    _watch() {
        try {
            ...
            console.error(`Jade AI Usage: cannot watch records: ${e.message}`);
        }
    }
    ...
    _isStale(record) {
        const interval = this._settings.get_int('refresh-minutes') * 60000;
    ...
    _runCollector(mode) {
        if (!GLib.file_test(this._collector, GLib.FileTest.EXISTS)) {
        ...
        const cancellable = this._cancellable;
        try {
            const proc = Gio.Subprocess.new(['python3', this._collector, mode],
            ...
            console.error(`Jade AI Usage: collector failed to start: ${e.message}`);
            if (mode === '--force') {
                this._refreshing = false;
                this._refreshFailed = true;
                this._renderFooter();
    ...
    _buildMenu() {
        ...
        menu.box.add_style_class_name('osaka-ai-menu');
    ...
    _renderFooter() {
        ...
        const installed = GLib.file_test(this._collector, GLib.FileTest.EXISTS);
